...
export async function createDynamicRouteGuard(
  ...
  if (!route.isInitAuthRoute) {
    ...
    if (!isLogin) {
      if (to.name === Enum_Route.Login) {
        next();
      } else {
        const redirect = to.fullPath;
        next({ name: Enum_Route.Login, query: { redirect } });
      }
      return false;
    }
    ...
    if (to.name === Enum_Route.NotFound) {
      // 动态路由没有加载导致被not-found路由捕获，等待权限路由加载好了，回到之前的路由
      // 若路由是从根路由重定向过来的，重新回到根路由
      const path = to.redirectedFrom?.name === Enum_Route.Root ? '/' : to.fullPath;
