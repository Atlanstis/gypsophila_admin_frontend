...
import { mhxyAreaList } from '@/service';
...
export function useChannelTable(
  onChannelEdit: (row: ApiMhxy.Area) => void,
  onChannelDelete: (row: ApiMhxy.Area) => void,
) {
  const { bool: loading, setTrue: startLoading, setFalse: endLoading } = useBoolean(true);

  const tableData = ref<ApiMhxy.Area[]>([]);

  async function getTableData() {
    startLoading();
    const { data, error } = await mhxyAreaList();
    if (!error) {
  ...
  const columns: Ref<DataTableColumns<ApiMhxy.Area>> = ref([
    {
      key: 'name',
      title: '名称',
    },
