    ...
    <TableContainer>
      ...
      <template #content>
        <NDataTable
          ...
          striped
          remote
          :row-key="(row: ApiMhxy.Channel) => row.id"
          :loading="loading"
          :columns="columns"
          :data="tableData"
        ></NDataTable>
...
import { mhxyAreaDelete } from '@/service';
...
function onChannelEdit(data: ApiMhxy.Area) {
  setEditData(data);
  setModalType('edit');
  openModal();
}

async function onChannelDelete(record: ApiMhxy.Area) {
  const { error } = await mhxyAreaDelete(record.id);
  if (!error) {
